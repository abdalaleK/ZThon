...
Zed_Vip = (6700336976, 6700336976, 6550930943, 5993018048, 5809896714, 1985225531, 6579579366, 6886550001, 925972505, 6038435721, 5746412340, 1762269116, 6272130846, 1052790413, 6055956182, 5059075331, 6669333713, 6328317500, 5616315677, 6227985448, 232499688)
zchannel = {"@", "@", "@", "@", "@", "@", "@", "@", "@", "@", "@", "@", "@", "@"}
...
async def mybot(): #Code by T.me/zzzzl1l
    ...
    if bot_name.startswith("مسـاعـد"):
        ...
        try:
            ...
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", f"- بـوت زدثـــون المسـاعـد ♥️🦾 الخـاص بـ  {bot.me.first_name} ")
            await asyncio.sleep(3)
    ...
    try:
        if BOTLOG:
            Config.ZEDUBLOGO = await zedub.tgbot.send_file(
                ...
                buttons=[(Button.url("الزعيم𓅛", "https://t.me/T_5_G"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
